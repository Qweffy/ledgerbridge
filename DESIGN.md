...
## Testing strategy
**79 tests** (Vitest) run against an **in-process Postgres (PGlite)** with the real migrations applied and a **fake QBO boundary** (`createFakeQbo` records what the processor asked it to do), so they exercise the production schema + the real worker/reconciler logic with no Docker or network. What they prove:
...
- **Full QBO entity coverage** beyond invoices / payments / accounts, and **reverse Payment sync** (QBO → internal).
